...
class CreateServiceChainSpecForm(forms.SelfHandlingForm):
	...
	try:
            ...
            self.fields['nodes'].choices = sorted(node_list, key=lambda obj: obj[1])
	except Exception:
...
class UpdateServiceChainSpecForm(CreateServiceChainSpecForm):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        try:
            ...
            nodes_selected = [(x,nodes_available[x]) for x in self.fields['nodes'].initial]
            nodes_unselected = set(self.fields['nodes'].choices) - set(nodes_selected)
            self.fields['nodes'].choices = nodes_selected + list(nodes_unselected)
        except Exception:
            msg = _("Failed to retrieve service chain spec")
            LOG.error(msg)
